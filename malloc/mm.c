...
void *mm_realloc(void *ptr, size_t size)
{
    ...
    if ((copySize = GET_SIZE(HDRP(oldptr))) >= size){
    	return oldptr;
    }
    else if(!GET_ALLOC(HDRP(NEXT_BLKP(ptr))) && 
    	(copySize = GET_SIZE(HDRP(ptr))+GET_SIZE(HDRP(NEXT_BLKP(ptr)))) >= size){
    	PUT(FTRP(ptr),0);
    	...
    	(copySize = GET_SIZE(HDRP(ptr))+GET_SIZE(HDRP(PREV_BLKP(ptr)))) >= size){
    	size = GET_SIZE(HDRP(ptr)); //此处为oldsize
    	PUT(FTRP(ptr),PACK(copySize,1));
    	PUT(HDRP(PREV_BLKP(ptr)),PACK(copySize,1));
    	newptr = PREV_BLKP(ptr);
    	memmove(newptr,oldptr,size);
    }
  	else{
  		newptr = mm_malloc(size);
    	memcpy(newptr, oldptr, GET_SIZE(HDRP(ptr)));
    	mm_free(oldptr);
    }
    return newptr;
}
